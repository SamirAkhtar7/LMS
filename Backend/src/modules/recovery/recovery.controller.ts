...
export const payRecoveryAmountConstroller = async (
    req: Request,
    res: Response
) => {
...
export const updateRecoveryStageController = async(
    ...
    try {
        const recovery = await updatateRecoveryStageService(
            recoveryId,
...
export const getLoanWithRecoveryController = async (
    ...
    try {
        ...
        res.status(500).json({
            success: false,
            message: error.message || "Failed to retrieve loans with recovery records",
            error: error.message,
        });
    }
}
